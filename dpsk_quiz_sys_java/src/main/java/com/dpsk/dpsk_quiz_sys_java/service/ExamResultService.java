...
public class ExamResultService implements IExamResultService {
    ...
    public ExamResultResponse submitExamAnswers(ExamAnswerSubmitRequest submitRequest) {
        ...
        try {
            ...
            // 获取试卷所有题目
            logger.info("[DEBUG] 正在查询试卷题目，paperId: {}", submitRequest.getPaperId());
            List<ExamQuestion> questions = examQuestionRepository.findByPaperIdOrderBySort(submitRequest.getPaperId());
            logger.info("[DEBUG] 查询到题目数量: {}", questions.size());
            if (questions.isEmpty()) {
                logger.error("[ERROR] 试卷没有题目，paperId: {}", submitRequest.getPaperId());
    ...
    @Override
    public Double calculatePassRate_ljs(Long paperId, BigDecimal passScore) {
    ...
    @Override
    public Map<QuestionType, String> getQuestionTypeAccuracy(Long userId, Long paperId) {
        ...
        // 先统计试卷中所有题型的题目总数
        Map<QuestionType, Integer> totalCount = questions.stream()
        ...
        // 统计用户回答正确的题目数
        Map<QuestionType, AtomicInteger> correctCount = new HashMap<>();
