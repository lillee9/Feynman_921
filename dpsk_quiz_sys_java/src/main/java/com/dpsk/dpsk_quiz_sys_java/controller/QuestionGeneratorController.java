...
//                    pw.flush();
    ...
    private String buildSystemPrompt(String questionType, String difficulty) {
        ...
        prompt.append("\n\n【核心要求】确保每道题目考察的知识点完全不同，严格避免重复考察同一知识点或业务场景。");
        prompt.append("每道题目应聚焦于不同的专业领域，如：催收技巧、法律条款、风险识别、客户沟通、业务流程、合规要求等。");
    ...
    private String getJsonFormatExample(String questionType) {
        switch (questionType) {
            ...
            case "JUDGE":
                return "\n{\"questions\":[{\"id\":1,\"type\":\"JUDGE\",\"question\":\"题目内容\",\"answer\":\"true\",\"explanation\":\"解析\"}]}";
            default:
                return "";
        }
    }
    ...
    private List<QuestionResponse> parseTextResponse(String response, String questionType) {
        ...
        try {
            ...
            for (String line : lines) {
                ...
                // 提取选项（选择题）
                else if (("SINGLE_CHOICE".equals(questionType) || "MULTI_CHOICE".equals(questionType)) && isOption(line)) {
                    if (currentQuestion != null) {
    ...
    private boolean isValidQuestionType(String questionType) {
        return Arrays.asList("SINGLE_CHOICE", "MULTI_CHOICE", "SHORT_ANSWER", "FILL_BLANK", "JUDGE").contains(questionType);
    }
