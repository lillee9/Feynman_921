    ...
    <nav class="navbar">
      <div class="nav-content">
        ...
        <div class="nav-links">
          ...
          <router-link to="/exam-selection" class="nav-link"
            >技能测试</router-link
          >
          ...
          <div class="ranking-card">
            ...
            <div class="paper-list" v-loading="loading">
              ...
              <div v-else class="no-data-tip">
                <el-empty description="该用户暂无考试数据" />
              </div>
            </div>
            <el-dialog
                ...
                <div class="score-time-section">
                  <div class="score-time-card">
                    ...
                    <div class="stats-grid">
                      ...
                      <!-- 新增题型准确率统计 -->
                      <div class="stat-item small-stat">
                        <div class="stat-label">
                          {{
                            currentPaperDetail.questionType1 || "题型一"
                          }}准确率
                        </div>
                        <div class="stat-value">
                          {{ currentPaperDetail.accuracy1 || "0/0" }}
                        </div>
                      </div>
                      <div class="stat-item small-stat">
                        <div class="stat-label">
                          {{
                            currentPaperDetail.questionType2 || "题型二"
                          }}准确率
                        </div>
                        <div class="stat-value">
                          {{ currentPaperDetail.accuracy2 || "0/0" }}
                        </div>
                      </div>
                      <div class="stat-item small-stat">
                        <div class="stat-label">
                          {{
                            currentPaperDetail.questionType3 || "题型三"
                          }}准确率
                        </div>
                        <div class="stat-value">
                          {{ currentPaperDetail.accuracy3 || "0/0" }}
                        </div>
                      </div>
                    </div>
                  </div>
...
const fetchScoreData = () => {
  axios
    ...
    .then((response) => {
      ...
      console.log("score1111", paperId.value);
      console.log("response.data", response.data);

      currentPaperDetail.value = {
        questionType1: response.data.questionTypes[0]?.typeName,
        accuracy1: response.data.questionTypes[0]?.accuracy,
        questionType2: response.data.questionTypes[1]?.typeName,
        accuracy2: response.data.questionTypes[1]?.accuracy,
        questionType3: response.data.questionTypes[2]?.typeName,
        accuracy3: response.data.questionTypes[2]?.accuracy,
      };
      console.log("currentPaper.value", currentPaperDetail.value);
    })
    .catch((error) => {
...
const beforeAnalyze = async () => {
  console.log("isAnalyzed.value", isAnalyzed);
  if (isAnalyzed.value) {
    try {
      await ElMessageBox.confirm("再次分析会覆盖历史记录，是否继续？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      });
      // 用户点击了"确定"，继续执行分析逻辑
      await analyzeAnswers(true);
    } catch (error) {
      // 用户点击了"取消"，什么都不做（或可以加提示）
      console.log("用户取消了操作");
    }
  } else {
    // 第一次分析，直接执行
    await analyzeAnswers(false);
  }
};
// 分析答卷
const analyzeAnswers = async (flag) => {
  ...
  if (!flag) {
    try {
      ...
      if (checkData.canAnalyze) {
        // 获取历史分析记录
        const { data: recordData } = await axios.get("/api/exam-analysis/get", {
          params: {
            user_id: userId.value, // 匹配后端@JsonProperty("user_id")
        ...
        analysisResult.value = {
          weakPoints: recordData.analysis_data,
          suggestions: recordData.advice_data,
        };
  ...
  const url = `/api/exam-analysis/analyze?userId=${userId.value}&paperId=${paperId.value}&flag=${flag}`;
  ...
  eventSource.value.onmessage = (event) => {
    ...
    if (event.data === "end") {
      ...
      // 新增保存到数据库的逻辑
      axios
        .post("/api/exam-analysis/save", {
          user_id: userId.value,
          paper_id: paperId.value,
          analysis_data: JSON.stringify(analysisResult.value.weakPoints),
          advice_data: JSON.stringify(analysisResult.value.suggestions),
        })
        .then((response) => {
...
/* 新增题型统计样式 */
.stats-grid {
  grid-template-columns: repeat(2, 1fr) repeat(3, minmax(150px, 1fr));
}
