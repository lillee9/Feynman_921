  ...
  eventSource.value.onmessage = (event) => {
    ...
    if (event.data === "end") {
      ...
      try {
        ...
        console.log("解析后的分析结果:", result); // 添加结果结构日志
        ...
        if (result.weakPoints) {
          ...
          console.log("更新后的知识点分析:", analysisResult.value.weakPoints);
        }
        ...
        if (result.suggestions) {
          const content = Array.isArray(result.suggestions)
            ...
            : result.suggestions;
          analysisResult.value.suggestions += content; // 字符串拼接
          console.log("更新后的学习建议:", analysisResult.value.suggestions);
        }
      } catch (e) {
