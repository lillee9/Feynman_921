      ...
      <div class="upper-section">
        ...
        <div class="info-section">
          <div class="update-info">
            <span>数据更新时间：{{ infoChart.value.updateTime }}</span>
            <span>总参与人数：{{ chartData.totalUsers }}</span>
            <span>昨日答题人数：{{ chartData.yesterdayUsers }}</span>
          </div>
        </div>
...
const fetchChartData = async () => {
  try {
    ...
    console.log("获取数据成功111121:", chartData.value.totalUsers);
    infoChart.value.totalUsers = chartData.value.totalUsers;
    infoChart.value.updateTime = chartData.value.updateTime;
    infoChart.value.yesterdayUsers = chartData.value.yesterdayUsers;
    // 更新图表
