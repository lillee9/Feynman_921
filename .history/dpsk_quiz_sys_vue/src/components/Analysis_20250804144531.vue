    ...
    if (event.data === "end") {
      ...
      try {
        ...
        if (result.content) {
          ...
          // 定义完整标记（使用正则匹配确保边界）
          const knowledgeRegex = /【欠缺知识点分析】/g;
          const suggestionRegex = /【用户建议】/g;

          // 循环处理所有标记（解决多个标记共存问题）
          let match;
          while ((match = knowledgeRegex.exec(tempBuffer.value)) !== null) {
            const splitIndex = match.index + match[0].length;
            const contentToAdd = tempBuffer.value.slice(splitIndex);
            analysisResult.value.weakPoints += contentToAdd;
            tempBuffer.value = tempBuffer.value.slice(0, match.index); // 保留标记前的残留内容
            console.log("更新知识点分析:", analysisResult.value.weakPoints);
          }

          while ((match = suggestionRegex.exec(tempBuffer.value)) !== null) {
            const splitIndex = match.index + match[0].length;
            const contentToAdd = tempBuffer.value.slice(splitIndex);
          ...
          // 若缓冲区仍有内容（未匹配到标记），保留待下次处理
          if (tempBuffer.value.length > 0) {
            console.log("未匹配到完整标记，保留缓冲区内容:", tempBuffer.value);
          }
        }
      } catch (e) {
