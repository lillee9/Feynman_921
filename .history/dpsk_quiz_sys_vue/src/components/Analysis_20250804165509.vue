...
const analyzeAnswers = () => {
  ...
  const processBufferContent = () => {
    ...
    const knowledgeRegex = /【用户欠缺知识点分析】([\s\S]*?)(?=【用户建议】|$)/g;
    const suggestionRegex = /【用户建议】([\s\S]*?)(?=【|$)/g;

    // 循环处理知识点分析（避免单次匹配遗漏）
    let knowledgeMatch;
    while ((knowledgeMatch = knowledgeRegex.exec(tempBuffer.value)) !== null) {
      analysisResult.value.weakPoints += knowledgeMatch[1].trim() + '\n';
      tempBuffer.value = tempBuffer.value.replace(knowledgeRegex, "");
      knowledgeRegex.lastIndex = 0; // 重置匹配位置
    }

    // 循环处理用户建议
    let suggestionMatch;
    while ((suggestionMatch = suggestionRegex.exec(tempBuffer.value)) !== null) {
