...
const analyzeAnswers = () => {
 ...
 try {
    const checkRes = await axios.get('/api/exam-analysis/makeSure', {
      params: { userId: userId.value, paperId: paperId.value }
    });

    if (checkRes.data.exists) {
      const result = await axios.get('/api/exam-analysis/get', {
        params: { userId: userId.value, paperId: paperId.value }
      });
