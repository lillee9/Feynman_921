    ...
    if (event.data === "end") {
      ...
      try {
        ...
        if (result.content) {
          ...
          console.log("当前临时缓冲区内容:", tempBuffer.value);
          ...
          // 检查临时缓冲区中是否包含完整标记
          if (tempBuffer.value.includes(knowledgeMarker)) {
            // 分割标记前后内容
            const parts = tempBuffer.value.split(knowledgeMarker);
            // 标记后的内容作为知识点分析内容
            const contentToAdd = parts.slice(1).join(knowledgeMarker);
            ...
            // 重置临时缓冲区
            tempBuffer.value = "";
          } else if (tempBuffer.value.includes(suggestionMarker)) {
            // 分割标记前后内容
            const parts = tempBuffer.value.split(suggestionMarker);
            // 标记后的内容作为用户建议内容
            const contentToAdd = parts.slice(1).join(suggestionMarker);
