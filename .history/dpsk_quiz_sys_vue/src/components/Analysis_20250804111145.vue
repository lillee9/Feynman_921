  ...
  eventSource.value.onmessage = (event) => {
    ...
    if (event.data === "end") {
      ...
      try {
        ...
        // 处理知识点分析 - 增加类型检查
        if (typeof result.weakPoints !== "undefined") {
          const content = Array.isArray(result.weakPoints)
            ? result.weakPoints.join("<br>")
            : typeof result.weakPoints === "string"
            ? result.weakPoints
            : "";
          ...
          console.warn("SSE消息缺少weakPoints字段");
        }

        // 处理学习建议 - 增加类型检查
        if (typeof result.suggestions !== "undefined") {
          const content = Array.isArray(result.suggestions)
          ...
          console.warn("SSE消息缺少suggestions字段");
        }
      } catch (e) {
