  ...
  const series = typeKeys.map((typeKey) => ({
    ...
    itemStyle: {
      color: (params) => {
        ...
        return undefined; // 默认色
      },
      borderColor: (params) => (params.data.noData ? "#bbb" : undefined),
      borderWidth: (params) => (params.data.noData ? 1 : 0),
      borderType: (params) => (params.data.noData ? "dashed" : "solid"),
    },
    label: {
      show: true,
      position: "top",
      color: (params) => (params.data.noData ? "#999" : "#333"),
      fontSize: (params) => (params.data.noData ? 11 : 12),
      formatter: (params) => {
        if (params.data.noData) return "暂无数据";
    ...
    tooltip: {
      ...
      formatter: (params) => {
        return params
          .map((p) => {
            if (p.data.noData) {
              return `${p.seriesName}（${p.name}）：暂无数据`;
            }
            return `${p.seriesName}（${p.name}）：${p.value.toFixed(2)}%`;
          })
          .join("<br/>");
      },
    },
