...
const analyzeAnswers = () => {
  if (analyzing.value) return; // 防止重复请求
  ...
  const eventSource = new EventSource(
    `/api/exam-analysis/analyze?userId=${userId.value}&paperId=${paperId.value}`
  );
  ...
  eventSource.onmessage = (e) => {
    ...
    if (data === "end") {
      // 分析结束
      eventSource.close();
      analysisResult.value = tempResults; // 将临时结果赋值给最终结果
      ElMessage.success("答卷分析完成");
      return;
    }

    try {
      const result = JSON.parse(data); // 解析后端返回的AnalysisContentDto
      // 根据大模型返回的内容格式，补充到临时结果（示例逻辑，需根据实际返回调整）
      if (result.content.includes("欠缺知识点")) {
        tempResults.weakPoints = result.content.split("：")[1].split("、");
      } else if (result.content.includes("学习建议")) {
        tempResults.suggestions = result.content.split("：")[1].split("；");
      }
      // 实时更新分析结果（可展示中间状态）
      analysisResult.value = { ...tempResults };
    } catch (error) {
