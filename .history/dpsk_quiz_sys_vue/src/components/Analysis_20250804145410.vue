    ...
    if (event.data === "end") {
      ...
      try {
        ...
        if (result.content) {
          ...
          // 定义标记（使用正则匹配确保边界）
          const knowledgeStart = "【欠缺知识点分析】";
          const suggestionStart = "【用户建议】";

          // 处理知识点分析内容
          let knowledgeIndex = tempBuffer.value.indexOf(knowledgeStart);
          while (knowledgeIndex !== -1) {
            const nextSuggestionIndex = tempBuffer.value.indexOf(
              suggestionStart,
              knowledgeIndex + knowledgeStart.length
            );
            const endIndex =
              nextSuggestionIndex !== -1
                ? nextSuggestionIndex
                : tempBuffer.value.length;
            const content = tempBuffer.value.slice(
              knowledgeIndex + knowledgeStart.length,
              endIndex
            );
            analysisResult.value.weakPoints += content;
            tempBuffer.value =
              tempBuffer.value.slice(0, knowledgeIndex) +
              tempBuffer.value.slice(endIndex);
            knowledgeIndex = tempBuffer.value.indexOf(knowledgeStart); // 继续查找下一个标记
          }

          // 处理学习建议内容（同理）
          let suggestionIndex = tempBuffer.value.indexOf(suggestionStart);
          while (suggestionIndex !== -1) {
            const content = tempBuffer.value.slice(
              suggestionIndex + suggestionStart.length
            );
            analysisResult.value.suggestions += content;
            tempBuffer.value = tempBuffer.value.slice(0, suggestionIndex);
            suggestionIndex = tempBuffer.value.indexOf(suggestionStart);
          }
        }
      } catch (e) {
