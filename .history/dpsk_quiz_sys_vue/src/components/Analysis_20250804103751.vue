          ...
          <div v-if="analysisResult" class="analysis-results">
            ...
            <div class="analysis-item">
              ...
              <div class="knowledge-content">
               <div class="knowledge-content" v-html="analysisResult.weakPoints"></div>
</div>
...
<div class="analysis-item">
  ...
  <div class="suggestions" v-html="analysisResult.suggestions"></div>
</div>
          </div>
        </div>
      </section>
  ...
  eventSource.value.onmessage = (event) => {
    ...
    if (event.data === "end") {
      ...
      try {
        ...
        if (result.content) {
          // 使用字符串拼接而非数组push
          analysisResult.value.weakPoints += result.content;
        } else {
          // 处理原始数据结构
          if (result.weakPoints && Array.isArray(result.weakPoints)) {
            analysisResult.value.weakPoints += result.weakPoints.join("");
          }
          if (result.suggestions && Array.isArray(result.suggestions)) {
            analysisResult.value.suggestions.push(...result.suggestions);
          }
        }
      } catch (e) {
