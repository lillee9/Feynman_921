...
const initBarChart = () => {
  ...
  const colors = ["#5470C6", "#91CC75", "#FAC858"]; // 每个系列的颜色
  ...
  const series = typeKeys.map((typeKey, seriesIndex) => ({
    ...
    itemStyle: {
      color: (params) => {
        if (params.data.noData) {
          return "#ccc"; // 灰色表示无数据
        }
        return colors[seriesIndex % colors.length]; // 正常颜色
      },
    },
    label: {
      show: true,
      position: "top",
      formatter: (params) => {
        return params.data.noData ? "Null" : `${params.data.value.toFixed(2)}`;
      },
    },
    barMaxWidth: 28,
  }));
...
const handleProfile = () => {
  try {
    if (!router) throw new Error("Router instance not available");
    ElMessage.info("正在进入个人中心...");
    router.push("/profile").catch((err) => {
      console.error("路由跳转失败:", err);
      ElMessage.warning("进入个人中心失败");
    });
  } catch (err) {
    console.error("个人中心操作异常:", err);
...
const handleLogout = async () => {
  try {
    ...
    console.error("退出登录流程失败:", err);
    ElMessage.error({
      title: "退出异常",
      message: err.message || "请手动清除浏览器缓存",
    });
