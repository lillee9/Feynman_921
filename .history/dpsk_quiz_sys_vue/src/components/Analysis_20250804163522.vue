...
const analyzeAnswers = () => {
  ...
  eventSource.value.onmessage = (event) => {
    ...
    if (event.data === "end") {
      ...
      // 最终检查：如果缓冲区还有内容，直接显示为建议
      if (tempBuffer.value.trim() && !analysisResult.value.suggestions) {
        console.log("将剩余缓冲区内容作为建议");
  ...
  const processBufferContent = () => {
    ...
    const knowledgeRegex =
      /(【用户欠缺知识点分析】|用户欠缺知识点分析)([\s\S]*?)(?=(【用户建议】|用户建议|$))/i;
    const suggestionRegex = /(【用户建议】|用户建议)([\s\S]*)/i;

    // 尝试匹配知识点分析
    const knowledgeMatch = tempBuffer.value.match(knowledgeRegex);
    if (knowledgeMatch && knowledgeMatch[2]) {
      console.log("找到知识点分析内容");
      analysisResult.value.weakPoints = knowledgeMatch[2].trim();
      console.log("知识点内容:", analysisResult.value.weakPoints);
    ...
    // 尝试匹配建议
    const suggestionMatch = tempBuffer.value.match(suggestionRegex);
    if (suggestionMatch && suggestionMatch[2]) {
      console.log("找到建议内容");
      analysisResult.value.suggestions = suggestionMatch[2].trim();
      console.log("建议内容:", analysisResult.value.suggestions);
