          ...
          <div class="analysis-results">
            ...
            <div class="analysis-item">
              ...
              <div
                ...
                v-html="analysisResult.weakPoints || '暂无欠缺知识点数据'
              ></div>
              <pre v-if="analysisResult.weakPoints || analyzing" class="debug-data">原始知识点数据: {{ analysisResult.weakPoints }}</pre>
            </div>
            ...
            <div class="analysis-item">
              ...
              <div
                class="suggestions"
                v-html="analysisResult.suggestions || '暂无学习建议数据'
              ></div>
              <pre v-if="analysisResult.suggestions || analyzing" class="debug-data">原始建议数据: {{ analysisResult.suggestions }}</pre>
            </div>
          </div>
        </div>
      </section>
  ...
  eventSource.value.onmessage = (event) => {
    ...
    if (event.data === "end") {
      ...
      try {
        ...
        console.log("解析后的分析结果结构:", Object.keys(result)); // 输出所有键名
        console.log("原始数据内容:", event.data);

        // 处理知识点分析 - 增加类型检查
        if (result && typeof result.weakPoints !== "undefined") {
          const content = result.weakPoints
            ? Array.isArray(result.weakPoints)
              ? result.weakPoints.join("<br>")
              : result.weakPoints
            : "";
          console.log("Processed weakPoints content:", content);
          analysisResult.value.weakPoints += content;
          console.log(
            "Updated weakPoints value:",
            analysisResult.value.weakPoints
          );
          console.log(
            "知识点分析长度:",
            analysisResult.value.weakPoints.length
          );
        } else {
        }
        ...
        if (result && typeof result.suggestions !== "undefined") {
          const content = result.suggestions
            ? Array.isArray(result.suggestions)
              ? result.suggestions.join("<br>")
              : result.suggestions
            : "";
          console.log("Processed suggestions content:", content);
          analysisResult.value.suggestions += content;
          console.log(
            "Updated suggestions value:",
            analysisResult.value.suggestions
          );
          console.log("学习建议长度:", analysisResult.value.suggestions.length);
        } else {
        }
      } catch (e) {
...
.suggestions {
  margin-top: 10px;
  line-height: 1.6;
  color: #333 !important; /* 确保文本可见 */
  min-height: 40px; /* 确保即使内容为空也有高度 */
  padding: 16px;
  border: 1px solid #ddd; /* 添加边框便于观察 */
}
.debug-data {
  margin-top: 10px;
  padding: 10px;
