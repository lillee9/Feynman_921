      ...
      <section class="analysis-section">
        <div class="analysis-card">
          ...
          <div v-if="analysisResult" class="analysis-results">
            ...
            <div
              class="analysis-item"
              v-if="analysisResult.weakPoints.length > 0"
            >
              <h3 class="analysis-title">欠缺知识点分析</h3>
              <div
                class="knowledge-content"
                v-html="analysisResult.weakPoints"
              ></div>
            </div>
            ...
            <div v-if="analysisResult.suggestions !== ''" class="analysis-item">
              ...
              <div
                class="suggestions"
                v-html="analysisResult.suggestions"
              ></div>
            </div>
          </div>
        </div>
...
const analyzeAnswers = () => {
  ...
  // 重置分析结果
  analysisResult.value.weakPoints = ""; // 保持字符串类型
  analysisResult.value.suggestions = ""; // 改为字符串类型
    ...
    if (event.data === "end") {
      ...
      try {
        ...
        if (result.content) {
          ...
          // 定义完整标记（使用正则匹配确保边界，每次创建新实例避免lastIndex干扰）
          const knowledgeRegex = new RegExp("【欠缺知识点分析】", "g");
          const suggestionRegex = new RegExp("【用户建议】", "g");

          // 处理知识点分析标记
          let knowledgeMatch;
          while (
            (knowledgeMatch = knowledgeRegex.exec(tempBuffer.value)) !== null
          ) {
            const splitIndex = knowledgeMatch.index + knowledgeMatch[0].length;
            const contentToAdd = tempBuffer.value.slice(splitIndex);
            // 检查contentToAdd是否包含其他标记，避免错误追加
            const nextSuggestionIndex = contentToAdd.indexOf("【用户建议】");
            if (nextSuggestionIndex !== -1) {
              analysisResult.value.weakPoints += contentToAdd.slice(
                0,
                nextSuggestionIndex
              );
              tempBuffer.value =
                tempBuffer.value.slice(0, knowledgeMatch.index) +
                contentToAdd.slice(nextSuggestionIndex);
            } else {
              analysisResult.value.weakPoints += contentToAdd;
              tempBuffer.value = tempBuffer.value.slice(
                0,
                knowledgeMatch.index
              );
            }
            console.log("更新知识点分析:", analysisResult.value.weakPoints);
          }

          // 处理学习建议标记（同理重置正则）
          let suggestionMatch;
          const newSuggestionRegex = new RegExp("【用户建议】", "g");
          while (
            (suggestionMatch = newSuggestionRegex.exec(tempBuffer.value)) !==
            null
          ) {
            const splitIndex =
              suggestionMatch.index + suggestionMatch[0].length;
            const contentToAdd = tempBuffer.value.slice(splitIndex);
            analysisResult.value.suggestions += contentToAdd;
            tempBuffer.value = tempBuffer.value.slice(0, suggestionMatch.index);
            console.log("更新学习建议:", analysisResult.value.suggestions);
          }
        }
      } catch (e) {
