...
const analyzeAnswers = () => {
  ...
  const processBufferContent = () => {
    ...
    const buffer = tempBuffer.value;

    const knowledgeStart = buffer.indexOf("【用户欠缺知识点分析】");
    const suggestionStart = buffer.indexOf("【用户建议】");

    let knowledgeContent = "";
    let suggestionContent = "";

    // 同时包含两个字段，精确分割
    if (knowledgeStart !== -1 && suggestionStart !== -1) {
      knowledgeContent = buffer
        .substring(
          knowledgeStart + "【用户欠缺知识点分析】".length,
          suggestionStart
        )
        .trim();
      suggestionContent = buffer
        .substring(suggestionStart + "【用户建议】".length)
        .trim();
    } else if (knowledgeStart !== -1) {
      // 仅有知识点分析
      knowledgeContent = buffer
        .substring(knowledgeStart + "【用户欠缺知识点分析】".length)
        .trim();
    } else if (suggestionStart !== -1) {
      // 仅有建议
      suggestionContent = buffer
        .substring(suggestionStart + "【用户建议】".length)
        .trim();
    ...
    if (knowledgeContent) {
      ...
      console.log("✅ 提取到知识点分析：", knowledgeContent);
    }

    if (suggestionContent) {
      analysisResult.value.suggestions = suggestionContent;
      console.log("✅ 提取到学习建议：", suggestionContent);
    }
