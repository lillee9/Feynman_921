<template>
  <div class="chart-container">
    ...
    <el-row :gutter="20" style="margin-top: 30px">
      <el-col :span="8">
        <div class="chart-card">
          <h3 class="chart-title">各题型难度分布</h3>
          <div ref="questionDifficultyBar" class="chart-placeholder"></div>
        </div>
      </el-col>
    </el-row>
  </div>
...
const questionDifficultyBar = ref(null);
// 新增API请求方法
const fetchChartData = async () => {
  try {
    ...
    initStackedBarChart(
      questionDifficultyBar.value,
      chartData.value.difficultyDistribution
    );
  } catch (error) {
...
// 堆叠柱状图初始化函数
const initStackedBarChart = (dom, data) => {
  const chart = echarts.init(dom);
  chart.setOption({
    xAxis: { type: "category", data: ["单选", "多选", "填空", "简答"] },
    yAxis: { type: "value", name: "题数" },
    series: [
      { name: "简单", type: "bar", data: data.easy, stack: "total" },
      { name: "中等", type: "bar", data: data.medium, stack: "total" },
      { name: "困难", type: "bar", data: data.hard, stack: "total" },
    ],
    legend: { top: "bottom" },
  });
};
