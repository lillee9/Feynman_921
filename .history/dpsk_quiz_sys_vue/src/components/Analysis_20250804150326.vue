...
const analyzeAnswers = () => {
  ...
  analysisResult.value = { weakPoints: "", suggestions: "" };
  tempBuffer.value = "";
  ...
  eventSource.value.onmessage = (event) => {
    ...
    try {
      ...
      if (result.content) {
        ...
        // 定义分隔符
        const knowledgeDelimiter = "【用户欠缺知识点分析】";
        const suggestionDelimiter = "【用户建议】";

        // 检查是否包含知识点分析部分
        if (tempBuffer.value.includes(knowledgeDelimiter)) {
          const parts = tempBuffer.value.split(knowledgeDelimiter);
          // 第一部分可能是空或之前的内容
          if (parts.length > 1) {
            // 检查是否也包含建议分隔符
            const knowledgeParts = parts[1].split(suggestionDelimiter);
            analysisResult.value.weakPoints = knowledgeParts[0].trim();

            // 如果有建议部分
            if (knowledgeParts.length > 1) {
              analysisResult.value.suggestions = knowledgeParts[1].trim();
              tempBuffer.value = ""; // 清空缓冲区
            } else {
              // 只处理了知识点部分，保留剩余内容
              tempBuffer.value = parts[1];
            }
          }
        }
        // 单独检查建议部分（可能先收到建议）
        else if (tempBuffer.value.includes(suggestionDelimiter)) {
          const parts = tempBuffer.value.split(suggestionDelimiter);
          if (parts.length > 1) {
            analysisResult.value.suggestions = parts[1].trim();
            tempBuffer.value = ""; // 清空缓冲区
          }
        }
      }
