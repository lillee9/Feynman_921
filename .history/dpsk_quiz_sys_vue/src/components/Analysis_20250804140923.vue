...
// 添加状态变量用于跟踪标记拼接状态
const contentBuffer = ref(""); // 内容缓冲区
const currentSection = ref(null); // 当前区域：'weakPoints' | 'suggestions' | null
  ...
  eventSource.value.onmessage = (event) => {
    ...
    if (event.data === "end") {
      ...
      try {
        if (result.content) {
          // 将新内容追加到缓冲区
          contentBuffer.value += result.content;
          console.log("当前缓冲区内容:", contentBuffer.value);

          // 定义标记正则表达式（支持部分匹配）
          const knowledgeMarker = "【欠缺知识点分析】";
          const suggestionMarker = "【用户建议】";

          // 检查缓冲区中是否包含完整标记
          if (contentBuffer.value.includes(knowledgeMarker)) {
            // 切换到知识点分析区域
            currentSection.value = "weakPoints";
            // 提取标记后的内容
            const parts = contentBuffer.value.split(knowledgeMarker);
            contentBuffer.value = parts.slice(1).join(knowledgeMarker); // 保留剩余内容
            analysisResult.value.weakPoints += contentBuffer.value;
            console.log(
              "检测到知识点标记，更新内容:",
              analysisResult.value.weakPoints
            );
            contentBuffer.value = ""; // 重置缓冲区
          } else if (contentBuffer.value.includes(suggestionMarker)) {
            // 切换到建议区域
            currentSection.value = "suggestions";
            // 提取标记后的内容
            const parts = contentBuffer.value.split(suggestionMarker);
            contentBuffer.value = parts.slice(1).join(suggestionMarker); // 保留剩余内容
            analysisResult.value.suggestions += contentBuffer.value;
            console.log(
              "检测到建议标记，更新内容:",
          ...
          // 如果已有活跃区域，则直接追加内容
            ...
            analysisResult.value[currentSection.value] += result.content;
            console.log(`追加${currentSection.value}内容:`, result.content);
          }
          // 否则继续累积缓冲区
          else {
            console.log("未检测到完整标记，继续累积缓冲区");
          }
        }
      } catch (e) {
