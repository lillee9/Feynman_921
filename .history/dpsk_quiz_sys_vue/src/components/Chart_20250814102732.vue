...
const initLineChart = (dom, data) => {
  ...
  const option = {
    ...
    xAxis: {
      type: "category",
      data: chartData.lineChart.dates, // 改为日期数组
      axisLabel: { fontWeight: "bold" },
    },
    yAxis: {
      type: "value",
      axisLabel: { formatter: "{value}人" }, // 修改单位
      max: Math.max(...chartData.lineChart.dailyParticipants) + 10,
    },
    series: [
      {
        name: "答题人数",
        type: "line",
        data: chartData.lineChart.dailyParticipants, // 改用答题人数数据
        smooth: true,
        symbol: "circle",
        symbolSize: 8,
