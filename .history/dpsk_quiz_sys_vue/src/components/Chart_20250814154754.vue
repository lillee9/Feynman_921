  ...
  <div class="chart-container">
    ...
    <div class="bar-chart-section">
      <div class="chart-card">
        ...
        <div ref="barChart" class="chart-placeholder" style="height: 300px;"></div>
      </div>
    </div>
  </div>
...
const initBarChart = () => {
  ...
  const series = typeKeys.map((typeKey) => ({
    ...
    data: difficulties.map((diffKey) => {
      const val = chartData.value[diffKey]?.[typeKey] ?? -1;
      return val >= 0 ? val : 0; // echarts 必须要数值，-1 替换成 0
    }),
    itemStyle: {
      color: (params) => {
        const val = chartData.value[difficulties[params.dataIndex]]?.[typeKey];
        return val >= 0 ? undefined : "#ccc"; // 灰色占位
      },
    },
    label: {
      show: true,
      ...
      formatter: (params) => {
        const val =
          chartData.value[difficulties[params.dataIndex]]?.[typeKey] ?? -1;
        return val >= 0 ? `${val.toFixed(2)}%` : "暂无数据";
      },
    },
  }));
  ...
  const option = {
    ...
    legend: { data: Object.values(typeLabels) },
