...
const initBarChart = () => {
  ...
  const series = typeKeys.map((typeKey) => ({
    ...
    data: difficulties.map((diffKey) => {
      ...
      if (val === -1) {
        return { value: 0, noData: true }; // 暂无数据：柱子高度为0，但标记 noData
      }
      return { value: val, noData: false };
    }),
    itemStyle: {
      color: (params) => (params.data.noData ? "#ddd" : undefined), // 暂无数据用灰色
    },
    label: {
      show: true,
      position: "top",
      formatter: (params) => {
        if (params.data.noData) return "{gray|暂无数据}";
        if (params.data.value === 0) return "{blue|0.00%}";
        return `${params.data.value.toFixed(2)}%`;
      },
      rich: {
        gray: { color: "#999", fontSize: 12 },
        blue: { color: "#409EFF", fontSize: 12 },
      },
    },
    barMaxWidth: 28,
  }));
