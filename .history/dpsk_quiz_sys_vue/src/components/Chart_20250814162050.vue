  ...
  const series = typeKeys.map((typeKey, seriesIndex) => ({
    ...
    label: {
      ...
      formatter: (params) => {
        ...
        // 修改2：0显示为"0"而不是"0.00"
        return params.data.value === 0 ? "0" : params.data.value.toFixed(2);
      },
      // 添加富文本样式
      rich: {
        noData: {
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        params.forEach((param) => {
          ...
          if (data.noData) {
            result += `${param.seriesName}: <span style="color:#999">暂无数据</span><br/>`;
          } else {
            // 修改1：移除百分号
            result += `${param.seriesName}: <span style="color:${
              param.color
            }">${data.value.toFixed(2)}</span><br/>`;
          }
        });
        return result;
      },
