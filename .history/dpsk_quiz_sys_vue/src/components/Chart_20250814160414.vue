...
const initBarChart = () => {
  ...
  const series = typeKeys.map((typeKey) => ({
    ...
    data: difficulties.map((diffKey) => {
      const val = chartData.value.barChart[diffKey]?.[typeKey] ?? -1;
      return val >= 0 ? val : 0; // echarts 必须要数值，-1 替换成 0
    }),
    itemStyle: {
      color: (params) => {
        const val =
          chartData.value.barChart[difficulties[params.dataIndex]]?.[typeKey];
  ...
  const option = {
    ...
    yAxis: {
      type: "value",
      axisLabel: {
        formatter: (value) => (value === -1 ? "N/A" : `{value}%`),
      },
      min: 0,
      max: 100,
    },
    series: series.map((s) => ({
      ...
      barMinHeight: 2, // 确保0值可见
    })),
    tooltip: {
      formatter: (params) => {
        if (params[0].value === null) return "暂无数据";
