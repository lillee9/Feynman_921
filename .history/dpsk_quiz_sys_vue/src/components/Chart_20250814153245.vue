...
const initBarChart = () => {
  ...
  // 获取所有题型
  const questionTypes = Object.keys(chartData.value.barChart.easy);

  const series = [
    {
      name: "简单",
      type: "bar",
      data: questionTypes.map((t) => chartData.value.barChart.easy[t] || 0),
    },
    {
      name: "中等",
      type: "bar",
      data: questionTypes.map((t) => chartData.value.barChart.medium[t] || 0),
    },
    {
      name: "困难",
      type: "bar",
      data: questionTypes.map((t) => chartData.value.barChart.hard[t] || 0),
    },
  ];
