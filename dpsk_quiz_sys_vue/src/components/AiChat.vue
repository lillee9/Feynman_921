...
const loadRecentConversations = async () => {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/conversation/getAll?userId=${userStore.id}`,
      {
        method: "GET",
        headers: {
...
const confirmRename = async () => {
  ...
  const response = await fetch(`${API_BASE_URL}/conversation/edit`, {
    method: "POST",
...
const loadConversationMessages = async (conversationId) => {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/message/getAll?conversationId=${conversationId}`
    );
...
const sendMessage = async (message) => {
  ...
  console.log(chats.value);
  // 使用fetch API处理流式响应，避免URL长度限制
  const requestBody = {
    messages: JSON.stringify(messages),
    conversationId: Number(activeConversation.value),
  };

  console.log("发送流式请求到:", `${API_BASE_URL}/stream`);
  ...
  try {
    ...
    if (lastMessage) {
      lastMessage.status = "error";
      lastMessage.content = "请求失败，请检查网络连接并重试";
    }
    isGenerating.value = false;
    scrollToBottom();
  }
