        ...
        <div class="paper-card current-paper">
          <div class="paper-header">
            ...
            <div class="paper-meta">
              <span class="paper-info">{{ currentPaper.questions.length }}道题</span>
              <span class="paper-info">{{ currentPaper.difficulty || '中等' }}</span>
              <span class="paper-info">{{ currentPaper.type || '综合练习' }}</span>
            </div>
          </div>
          ...
          <div class="questions-preview">
            ...
            <div class="questions-list">
              <div 
                ...
                <div class="question-content">
                  <div class="question-text">{{ question.question || question.questionContent || question.content }}</div>
            ...
            <div v-if="currentPaper.questions.length > 3" class="more-questions">
              还有 {{ currentPaper.questions.length - 3 }} 道题目...
            </div>
          </div>
          
          <div class="paper-actions">
            <el-button type="primary" size="large" @click="startExam(currentPaper)">
      ...
      <div v-if="selectedPaper" class="paper-detail">
        <div class="detail-meta">
          <div class="meta-item">
            ...
            <span class="meta-value">{{ selectedPaper.totalQuestion || selectedPaper.questions?.length }}道</span>
          </div>
          <div class="meta-item">
          ...
          <div class="meta-item">
            ...
            <span class="meta-value">{{ formatDate(selectedPaper.createTime) }}</span>
          </div>
        </div>
...
const loadPaperData = async (paperId) => {
  ...
  try {
    ...
    if (paperResult.code !== 200) {
      ElMessage.error('获取试卷基本信息失败：' + (paperResult.message || '未知错误'))
      return
    }
    
    // 然后获取试卷题目列表
    const questionsResult = await authFetch(`${API_BASE_URL}/exam-question/paper/${paperId}`, {
      method: 'GET'
    }, router)
...
const generatePaperDescription = (paperData) => {
  return `题目类型：${paperData.type || '综合'}，难度：${paperData.difficulty || '中等'}，题目数量：${paperData.total || paperData.questions?.length || 0}道。`
}
...
const startExam = (paper) => {
  ...
  const validId = possibleIds.find(id => id !== undefined && id !== null && id !== '')
...
const viewFullPaper = async (paper) => {
  if (paper.questions) {
    ...
    try {
      const result = await authFetch(`${API_BASE_URL}/exam-question/paper/${paper.id}`, {
        method: 'GET'
      }, router)
