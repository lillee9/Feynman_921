              ...
              <div v-if="papers.length > 0">
                <div v-for="paper in papers" :key="paper.id" class="paper-item">
                  <div class="paper-info" @click="showPaperDetails(paper)">
                    ...
                    <p class="paper-types" v-if="paper.questionTypes && paper.questionTypes.length > 0">
                      题型：{{ paper.questionTypes.join('、') }}
                    </p>
                ...
                <div class="score-time-section">
                  <div class="score-time-card">
                    ...
                    <div class="stats-grid">
                      ...
                      <!-- 新增题型准确率统计 -->
                      <div
                        class="stat-item small-stat"
                        v-for="(qt, index) in currentPaperDetail.questionTypes"
                        :key="index"
                      >
                        <div class="stat-label">
                          {{ qt.typeName || `题型${index + 1}` }} 准确率
                        </div>
                        <div class="stat-value">
                          {{ qt.accuracy || "0/0" }}
                        </div>
...
onMounted(() => {
  ...
  fetchPapers();
  // fetchScoreData() 移除 - 只在用户选择具体试卷时调用
});
...
const fetchPapers = async () => {
  ...
  try {
    ...
    // 按endTime降序排序，最新的答题记录在前
    papers.value = response.data.sort((a, b) => {
...
const fetchScoreData = () => {
  axios
    ...
    .then((response) => {
      score.value = response.data.totalScore;
      ...
      currentPaperDetail.value = {
        ...
        paperDifficulty: response.data.paperDifficulty,
      ...
      console.log("questionTypes数据:", response.data.questionTypes);
    })
    .catch((error) => {
...
.stats-grid {
  display: grid;
  grid-template-columns: repeat(2, 1fr) repeat(3, minmax(180px, 1fr));
  gap: 20px;
  margin-top: 32px;
  align-items: stretch;
}
