    ...
    <div class="main-content">
      ...
      <div class="content-area">
        ...
        <div v-if="questions.length > 0" class="question-card">
          ...
          <!-- 判断题选项 -->
          <div v-else-if="['JUDGE', 'judge', '判断题', 'TRUE_FALSE', 'BOOLEAN'].includes(currentQuestion.type || currentQuestion.questionType)">
            <div class="options-container">
              <div class="option-item">
                <input
                  :id="`judge-true-${currentQuestionIndex}`"
                  type="radio"
                  :name="`judge-${currentQuestionIndex}`"
                  value="true"
                  v-model="userAnswers[currentQuestionIndex]"
                  class="option-input"
                />
                <label :for="`judge-true-${currentQuestionIndex}`" class="option-label">
                  <span class="option-text">正确</span>
                </label>
              </div>
              <div class="option-item">
                <input
                  :id="`judge-false-${currentQuestionIndex}`"
                  type="radio"
                  :name="`judge-${currentQuestionIndex}`"
                  value="false"
                  v-model="userAnswers[currentQuestionIndex]"
                  class="option-input"
                />
                <label :for="`judge-false-${currentQuestionIndex}`" class="option-label">
            ...
            <div class="score-display">
              <div class="score-grid">
                <div class="score-item">
                  <div class="score-number score-percentage">
                    {{ Math.round((correctCount / questions.length) * 100) }}
                  </div>
                  <div class="score-label">总得分</div>
                </div>
                <div class="score-item">
                  <div class="score-number score-correct">
                    {{ correctCount }}
                  </div>
                  <div class="score-label">正确题数</div>
                </div>
                <div class="score-item">
                  <div class="score-number score-total">
                    {{ questions.length }}
                  </div>
          ...
          <div class="modal-content">
            <div class="questions-list">
              ...
              >
                ...
                <p class="question-text">{{ question.questionContent || question.question }}</p>
                ...
                <div class="answer-section">
                  <div class="answer-row">
                    ...
                    <span
                      ...
                      :class="{
                        ...
                        'wrong-answer': !isAnswerCorrect(index)
                      }"
                    >
                  ...
                  <div class="answer-row">
                    ...
                    <span class="correct-answer-text">{{ question.correctAnswer || question.answer }}</span>
                  </div>
                  <div v-if="question.analysis || question.explanation" class="explanation-section">
                    <span class="explanation-label">解析:</span>
                    <span class="explanation-text">{{ question.analysis || question.explanation }}</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  try {
    ...
    if (route.query.paperId) {
      ...
      if (storedData) {
        ...
        // 检查考试状态（无论是否有试卷ID都要检查）
        const paperId = data.examPaperId || data.paperId
        if (paperId && userStore.id) {
          console.log('🔍 检查考试状态，试卷ID:', paperId)
          const examStatus = await checkExamStatus(paperId, userStore.id)
          if (examStatus.hasParticipated) {
            console.log('⚠️ 用户已参加过该试卷考试')
            const handleResult = await handleExistingExam(examStatus, paperId)
            if (!handleResult.allowContinue) {
          ...
          console.log('📋 跳过考试状态检查：缺少试卷ID或用户ID')
        }
